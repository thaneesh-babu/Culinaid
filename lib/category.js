    ...
    while (copy.length > 0) {
        ...
        if (current.children && Object.keys(current.children).length > 1) {
            ...
            for (const key in current.children) {
                if (key !== "array") {
                    let currentCopy = JSON.parse(
                        JSON.stringify(current.children[key])
                    );
    ...
    if (response.status == 200) {
        ...
        let promiseOne = currentResult.map(async (root) => {
            ...
            children.array.forEach((child) => {
                copied.children[child]["children"] =
                    parentCategoriesMap[root.nodeCode][child];
            });
...
export async function getCategoriesByParentId(parentId) {
    let response = await sendRequest(
        `${baseUrl}/category-nodes?parentId=${parentId}`,
        "GET"
    );
    return response;
}

export async function getCatalogItemCategoryAncestorsByMerchandiseCategory(
    nodeId
) {
    let response = await sendRequest(
        `${baseUrl}/category-nodes/${nodeId}/ancestors`,
        "GET"
    );
        ...
        while (hierarchicalNodes.length != nodes.length) {
            if (hierarchicalNodes.length == 0) {
                ...
                _.find(nodes, function (element) {
                    if (
                        element.parentId &&
                        element.parentId.nodeId == lastNode.nodeCode
                    ) {
                        hierarchicalNodes.push(element);
                        return false;
                    }
