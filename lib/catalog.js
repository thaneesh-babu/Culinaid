 ...
 */
export async function getSiteCatalogItemAttributesByItemCodes(
    siteId,
    itemCodes
) {
    return await sendRequest(
 ...
 * @param {string} itemCode itemCode of the catalog item
 */
 export async function getSiteCatalogItemAttributesByItemCode(siteId, itemCode) {
    return await sendRequest(
        `${baseUrl}/v2/item-attributes/${itemCode}`,
        "GET",
        null,
        siteId
    );
 ...
 * @returns {object} The response as { status: string, data: JSON }
 */
export async function getSiteCatelogItemsByMerchandiseCategoryId(
    siteId,
    category
) {
    return await sendRequest(
        `${baseUrl}/item-details/search?merchandiseCategoryId=${category}&itemStatus=ACTIVE&sortField=ITEM_CODE&sortDirection=ASC`,
        "GET",
        null,
        siteId
    );
 ...
 * @returns {object} The response as { status: string, data: JSON }
 */
export async function updateSiteCatalogItemPricesByItemCode(
    siteId,
    itemCode,
    priceCode,
    values
 ...
 * @returns {object} The response as { status: string, data: JSON }
 */
export async function updateSiteCatalogItemAttributesByItemCode(
    siteId,
    itemCode,
    values
) {
...
export async function getCatalogItemsByGroup(siteId, group) {
    return await sendRequest(
        `${baseUrl}/item-details/search?groupCode=${group}`,
        "GET",
        null,
        siteId
    );
...
export async function getItemDetail(siteId, itemId) {
    return await sendRequest(
        `${baseUrl}/item-details/search?itemId=${itemId}`,
        "GET",
        null,
        siteId
    );
